...
postQueue.on("waiting", (jobId) => {
  console.log(`[QUEUE DEBUG] Job ${jobId} is waiting to be processed`);
});

postQueue.on("active", (jobId) => {
  console.log(`[QUEUE DEBUG] Job ${jobId} has started processing`);
});

postQueue.on("completed", (jobId) => {
  console.log(`[QUEUE DEBUG] Job ${jobId} has completed successfully`);
});

postQueue.on("failed", (jobId, error) => {
  console.error(`[QUEUE DEBUG] Job ${jobId} has failed with error:`, error);
});

postQueue.on("stalled", (jobId) => {
  console.warn(`[QUEUE DEBUG] Job ${jobId} has stalled`);
});
